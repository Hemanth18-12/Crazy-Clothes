          ...
          <div style={{ background: 'var(--a-surface)', border: '1px solid var(--a-border)', width: '100%', maxWidth: '900px', position: 'relative' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', minHeight: '400px' }}>
              ...
              <div style={{ borderRight: '1px solid var(--a-border)', padding: '32px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                ...
                {detailModal.phone && (
                  <a href={`https://wa.me/91${detailModal.phone.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" style={{ color: '#25D366', fontFamily: 'var(--a-font-mono)', fontSize: '0.7rem', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                {detailModal.address && (
                  <div style={{ background: 'var(--a-surface2)', border: '1px solid var(--a-border)', padding: '12px', borderRadius: '4px', marginTop: '8px', marginBottom: '8px', textAlign: 'left', width: '100%' }}>
                    <div style={{ fontFamily: 'var(--a-font-mono)', fontSize: '0.6rem', color: 'var(--a-text3)', textTransform: 'uppercase', marginBottom: '6px' }}>Delivery Address</div>
                    <div style={{ fontFamily: 'var(--a-font-body)', fontSize: '0.8rem', color: 'var(--a-text)', lineHeight: '1.4' }}>
                      {[detailModal.address.houseNo, detailModal.address.street, detailModal.address.village].filter(Boolean).join(', ')}<br/>
                      {[detailModal.address.city, detailModal.address.state, detailModal.address.pincode].filter(Boolean).join(', ')}
                      {detailModal.address.landmark ? <><br/><span style={{color: 'var(--a-text2)', fontSize: '0.75rem'}}>Landmark: {detailModal.address.landmark}</span></> : null}
                    </div>
                  </div>
                )}
